...
public class Servidor extends Thread{
	...
	private boolean temp;//Resp�nsavel por determinar se o jogador existe ou n�o
	/**
	 * @param serverSocket
	 * @param porta
	 * @param nomeServidor
	...
	public Socket ouvirPorta() throws IOException {
		...
		telaServidor.getTextArea().append("<Houve uma requisi��o> \r\n");
				...
				while ((o=ois.readObject()) != null ) {
					if(o instanceof String) {
						...
						if(resp.contains(ProtocoloUtil.LISTA_USER_ONLINE)) {//
							if(!temp) {//se temp for falso significa que o jogador n�o exite o nome dele e setado e ele avisa aos demais
								
								System.out.println("Cl:"+cl.getNome());
								for(ClienteServer c:clientes) {//retorna pra mim os outros que j� estavam online
						...
						}else if(resp.contains(ProtocoloUtil.QUER_JOGAR)) {//enviar requisi��o para jogar partida
...
//							}
							...
							//	break;
						}else if(resp.contains(ProtocoloUtil.TERMINEI)) {//quando o ultimo jogador terminar de montar o mapa a requisi��o vem pra ca
							String []s=resp.split(ProtocoloUtil.SEPARADOR);
				...
				//pegar a exceção quando o cliente sair e avisar aos outros
